...
// NOTE: Notifications are cron-only — no import needed here.
...
export const verifyOtp = async (req, res) => {
  try {
    ...
    // NOTE: Notifications are handled exclusively by cron jobs — no login triggers.
...
export const resendOtp = async (req, res) => {
  try {
    ...
    return res.status(200).json({ message: "New OTP sent. Valid for 10 minutes." });
  } catch (err) {
...
export const login = async (req, res) => {
  try {
    ...
    const isLegacyUser = user.isVerified === undefined || user.isVerified === null;
