...
async function seedNews() {
  try {
    ...
    console.log(`📰 Only ${count} articles — fetching fresh news...`);
    const categories = [
    ...
    if (totalSaved === 0 && count === 0) {
      console.error("❌ No articles saved. Check NEWS_API_KEY in .env");
      return;
    }
    ...
    console.log(`✅ Seed complete — ${totalSaved} articles added.`);
  } catch (error) {
...
async function refreshAllNews() {
  console.log("⏰ Refreshing all news categories...");
  const categories = [
    "top", "technology", "business", "sports", "health",
    "science", "entertainment", "politics", "education", "world", "india",
  ];
  for (const cat of categories) {
    try {
      ...
      const saved = await saveNewsToDatabase(articles);
      console.log(`  ✔ ${cat}: ${saved.length} new articles.`);
...
mongoose
  ...
  .then(async () => {
    ...
    app.listen(PORT, () => {
      ...
      console.log("📅 News automation active (hourly refresh + alerts)");
    });
  })
  .catch((err) => {
