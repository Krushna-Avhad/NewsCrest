...
// Persona icon map — simple SVG icons, no emojis
const PERSONA_ICONS = {
...
export default function ArticleDetailPage() {
  ...
  const handleShare = () => {
    ...
    if (navigator.share) {
      navigator
        .share({ title: article?.title, text, url: article?.url || window.location.href })
        .catch(() => {});
    } else {
      ...
      <div className="grid grid-cols-[1fr_360px] gap-7">
          ...
          <div className="flex items-center gap-4 mb-5 pb-5 border-b border-gold/20">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                {article.author && (
                  <div className="text-[10px] text-text-muted">{article.author}</div>
                )}
              </div>
            </div>
        ...
        <div className="space-y-4 panel-slide-up">
          ...
          <div className="bg-white rounded-[14px] border border-gold-subtle p-[18px]">
            ...
            <Button variant="secondary" size="sm" className="w-full justify-center" onClick={() => {}}>
